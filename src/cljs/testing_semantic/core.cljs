...
(defn home-page []
  [:div {:class "ui large top fixed hidden menu"}
   [:div {:class "ui container"}
    ...
    [:a {:class "item"} "Work"]
    [:a {:class "item"} "Company"]
    [:a {:class "item"} "Careers"]
  ...
  [:div {:class "pusher"}
   [:div {:class "ui inverted vertical masthead center aligned segment"}
    ...
    [:div {:class "ui text container"}
     [:h1 {:class "ui inverted header"} "Example ThinkTopic Demo"]
    ...
    [:div {:class "ui middle aligned stackable grid container"}
     [:div {:class "row"}
      [:div {:class "eight wide column"}
       [:h3 {:class "ui header"} "We Help Companies and Companions"]
       ...
       [:h3 {:class "ui header"} "We Make Bananas That Can Dance"]
       [:p "Yes that&#39;s right, you thought it was the stuff of dreams, but even bananas can be bioengineered."]]
      [:div {:class "six wide right floated column"}
       [:img {:src "http://thinktopic.com/images/blog/hello-world-tt.jpg", :class "ui large bordered rounded image"}]]]
     [:div {:class "row"}
     ...
     [:div {:class "center aligned row"}
      ...
      [:div {:class "column"}
       ...
       [:p
        ...
        [:b "Nan"]" Chief Fun Officer Acme Toys"]]]]]
   [:div {:class "ui vertical stripe segment"}
    [:div {:class "ui text container"}
     [:h3 {:class "ui header"} "Breaking The Grid, Grabs Your Attention"]
     ...
     [:a {:class "ui large button"} "Read More"]
     [:h4 {:class "ui horizontal header divider"}
      [:a {:href "#"} "Case Studies"]]
     [:h3 {:class "ui header"} "Did We Tell You About Our Bananas?"]
     [:p "Yes I know you probably disregarded the earlier boasts as non-sequitur filler content, but its really true. It took years of gene splicing and combinatory DNA research, but our bananas can really dance."]
     [:a {:class "ui large button"} "I&#39;m Still Quite Interested"]]]
     ...
     [:div {:class "ui stackable inverted divided equal height stackable grid"}
      [:div {:class "three wide column"}
       ...
       [:div {:class "ui inverted link list"}
        ...
        [:a {:href "#", :class "item"} "Religious Ceremonies"]
        [:a {:href "#", :class "item"} "Gazebo Plans"]]]
      [:div {:class "three wide column"}
       [:h4 {:class "ui inverted header"} "Services"]
       [:div {:class "ui inverted link list"}
        [:a {:href "#", :class "item"} "Banana Pre-Order"]
        [:a {:href "#", :class "item"} "DNA FAQ"]
        [:a {:href "#", :class "item"} "How To Access"]
        [:a {:href "#", :class "item"} "Favorite X-Men"]]]
